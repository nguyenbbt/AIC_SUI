...
def test_qwen_vlm_sends_resized_local_evidence_and_validates_grounding(tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        assert request.headers["Authorization"] == "Bearer test-mgpux-key"
...
def test_qwen_vlm_rejects_blank_api_key(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="API key"):
        QwenVLMAdapter(data_root=tmp_path, api_key="   ")
